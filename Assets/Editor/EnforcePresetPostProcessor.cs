...
// Source: https://docs.unity3d.com/Manual/DefaultPresetsByFolder.html
...
namespace PresetsPerFolder
{
    ...
    {
        private static HashSet<string> s_pendingFolders;

        [RuntimeInitializeOnLoadMethod(RuntimeInitializeLoadType.SubsystemRegistration)]
        private static void OnDomainReload()
        {
            EditorApplication.delayCall -= OnDelayedCallback;
            s_pendingFolders = null;
        }

        private static void OnDelayedCallback()
        {
            EditorApplication.delayCall -= OnDelayedCallback;
            var pending = s_pendingFolders;
            s_pendingFolders = null;
            DelayedDependencyRegistration(pending);
        }
        ...
        protected override void OnAssetsModified(string[] changedAssets, string[] addedAssets, string[] deletedAssets, AssetMoveInfo[] movedAssets)
        {
            ...
            if (folders.Count != 0)
            {
                ...
                // Accumulate folders and register the static callback only once.
                if (s_pendingFolders == null)
                {
                    s_pendingFolders = folders;
                    EditorApplication.delayCall += OnDelayedCallback;
                }
                else
                {
                    foreach (var f in folders)
                        s_pendingFolders.Add(f);
                }
            }
        }
        /// <summary>
