...
- Added missing `.meta` file for `AssetSyncAPI.cs`.

## [1.7.0] - 2026-05-12

### Added

- Added `AssetSyncAPI` scripting API with `SyncAll()` and `SyncByName(string)` methods for triggering synchronization from editor scripts.

## [1.6.1] - 2026-05-06

### Changed

- Preserved cloned destination files when an external `Source` directory is not available on the current machine.
